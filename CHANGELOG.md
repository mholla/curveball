...
- **neighbor_info.py – Performance Optimization**  
  Original version bottlenecked entire pipeline due to nested loops scanning all triangles for every vertex → O(V × T) time complexity (V = vertices, T = triangles).  
...
- **Curvedness (CVD) Calculation – New Metric**  
...
- Initial pipeline focused on ABIDE dataset analysis: mean and gaussian curvature, thickness, sulcal depth, ICI/FI calculations.
