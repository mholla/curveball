...
def g_curv():
    ...
    # Plot    
    plt.figure()
    ax = sns.kdeplot(t_sulc_K, color='#31688E', shade=True, label='Pos k1, k2',bw_adjust=5, cut=0)
    ax = sns.kdeplot(t_neg_K, color='#FDE725', shade=True, label='Neg K', bw_adjust=5, cut=0)
    ax = sns.kdeplot(t_gyr_K, color='#31688E', shade=True, label='Neg k1, k2', bw_adjust=5, cut=0)
    ax.set(xlim=(0.5, 5))
    ax.set(xlim=(0.5, 5))
    ax.set(xlim=(0.5, 5))
    fname = '/afs/crc.nd.edu/group/commandlab/Nagehan/curveball_scripts/plots/results_all/g_curv_all.png'
    ...
    fname = '/afs/crc.nd.edu/group/commandlab/Nagehan/curveball_scripts/plots/results_all/g_curv_effect_all.png'
    plt.savefig(fname, dpi = 500)
    
    names = [('mean_gyr', 'mean_sulc', 'mean_saddle', 'std_gyr', 'std_sulc', 'std_saddle', 'p_val1', 'p_val2')]
    results = [(np.mean(t_gyr_K), np.mean(t_sulc_K), np.mean(t_neg_K), np.std(t_gyr_K), np.std(t_sulc_K), np.std(t_neg_K), p_val1, p_val2)]
        
    K_name = '/afs/crc.nd.edu/group/commandlab/Nagehan/curveball_scripts/plots/results_all/g_curv_all.asc'
