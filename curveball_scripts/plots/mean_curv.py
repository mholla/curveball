...
def mean_curv():
    ...
    plt.figure()
    ax = sns.kdeplot(t_sulc_H, color='#FDE725', shade=True, label='Positive H', bw_adjust=5, cut=0)
    bx = sns.kdeplot(t_gyr_H, color='#31688E', shade=True, label='Negative H', bw_adjust=5, cut=0)
    ax.set(xlim=(0.5, 5))
    bx.set(xlim=(0.5, 5))
    fname = '/afs/crc.nd.edu/group/commandlab/Nagehan/curveball_scripts/plots/results_all/mean_curv_all.png'
    ...
    plt.figure()
    ax = sns.stripplot(data=[d_H], linewidth=0.8, color='white', edgecolor='black', size=5, alpha=0.8)
    ax = sns.boxplot(data=[d_H], fliersize=0, linewidth=1.5, width = 0.25, color='lightgrey')
    ax.set(ylim=(0, 1.2))
    ax.tick_params(axis = "x", which = "both", bottom = False, top = False)
    fname = '/afs/crc.nd.edu/group/commandlab/Nagehan/curveball_scripts/plots/results_all/mean_curv_effect_all.png'
    plt.savefig(fname, dpi = 500)
    
    names = [('mean_gyr', 'mean_sulc', 'std_gyr', 'std_sulc', 'p')]
    results = [(np.mean(t_gyr_H), np.mean(t_sulc_H), np.std(t_gyr_H), np.std(t_sulc_H), p_val)]
        
    H_name = '/afs/crc.nd.edu/group/commandlab/Nagehan/curveball_scripts/plots/results_all/mean_curv_all.asc'
