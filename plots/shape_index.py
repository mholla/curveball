...
for all subjects. 
Effect sizes are calculated for each distribution combination
Parameters:
----------
...
Returns:
-------
    Distribution profile of shape index for nine different shapes along with effect size

"""

def shape_index(input_txt, subjects_name, output_folder):
    ...
    # Viridis Color Map
    plt.figure()
    ax = sns.kdeplot(t_cup_SI, color='#FDE725', shade=False, label='Cup',bw_adjust=2, cut=0)
    ax = sns.kdeplot(t_trough_SI, color='#B4DE2C', shade=False, label='Trough', bw_adjust=3,cut=0)
    ax = sns.kdeplot(t_rut_SI, color='#6DCD59', shade=False, label='Rut', bw_adjust=4,cut=0)
    ax = sns.kdeplot(t_srut_SI, color='#35B779', shade=False, label='Saddle_Rut', bw_adjust=4,cut=0)
    ax = sns.kdeplot(t_saddle_SI, color='#1F9E89', shade=False, label='Saddle', bw_adjust=4,cut=0)
    ax = sns.kdeplot(t_sridge_SI, color='#26828E', shade=False, label='Saddle_ridge', bw_adjust=4,cut=0)
    ax = sns.kdeplot(t_ridge_SI, color='#31688E', shade=False, label='Ridge', bw_adjust=4,cut=0)
    ax = sns.kdeplot(t_dome_SI, color='#3E4A89', shade=False, label='Dome', bw_adjust=3,cut=0)
    ax = sns.kdeplot(t_cap_SI, color='#482878', shade=False, label='Cap', bw_adjust=2,cut=0)
    ax.set(xlim=(0.5, 5))
