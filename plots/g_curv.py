...
- Effect sizes for each distribution combination are calculated for each subject

Parameters:
---------
    K = Gaussian curvature from {h}.pial.K.asc
    k1 = 1st principal curvature from {h}.pial.k1.asc
    k2 = 2nd principal curvature from {h}.pial.k2.asc
    t = cortical thickness from {h}.thickness.asc

Returns:
-------
    Distribution profiles along with statistical data and effect sizes
"""

def g_curv(input_txt, subjects_name, output_folder):
    ...
    for line in lines:
        ...
        for hemi in hemis:
            ...
            # Positive Gaussian Curvature divided into convex and concave points by using principal curvatures
    ...
    names = [('mean_gyr', 'mean_sulc', 'mean_saddle', 'std_gyr', 'std_sulc', 'std_saddle', 'p_val1', 'p_val2', 'd1_mean', 'd2_mean', 'd3_mean')]
